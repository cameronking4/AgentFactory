          ...
          <div className="flex-1">
            <div className="flex gap-2 items-center mb-2">
              ...
              {employeeDetails.employee.status === "terminated" && (
                <span className="px-3 py-1 rounded text-sm font-medium border bg-destructive/10 text-destructive border-destructive/20">
                  Terminated
                </span>
              )}
            </div>
            <h3 className="text-lg font-semibold text-foreground mb-2">{employeeDetails.employee.name}</h3>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="space-y-6">
            ...
            <div className="rounded-lg border bg-muted p-4">
              <div className="flex flex-wrap gap-2">
                {employeeDetails.employee.skills.map((skill, idx) => (
                  <span key={idx} className="px-2 py-1 rounded text-xs border bg-card">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          <div>
            ...
            <div className="rounded-lg border bg-muted p-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <div className="text-xs text-muted-foreground">Pings</div>
                  <div className="text-2xl font-bold text-foreground">{employeeDetails.stats.totalPings}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-3 max-h-[300px] overflow-y-auto">
                {employeeDetails.memories.slice(0, 10).map((memory) => (
                  <div key={memory.id} className="rounded-lg border bg-card p-4">
                    ...
                    <div className="text-sm mt-2 text-foreground line-clamp-3">{memory.content}</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-3 max-h-[300px] overflow-y-auto">
                {employeeDetails.pings.slice(0, 5).map((ping, idx) => (
                  <div key={idx} className="rounded-lg border bg-card p-4">
                    <div className="text-sm text-foreground line-clamp-3">{ping.content}</div>
          ...
          {employeeDetails.meetings.recent.length > 0 && (
            <div>
              <h4 className="text-sm font-semibold text-foreground mb-2">Recent Meetings</h4>
              <div className="space-y-3 max-h-[300px] overflow-y-auto">
                {employeeDetails.meetings.recent.slice(0, 5).map((meeting) => (
                  <div key={meeting.id} className="rounded-lg border bg-card p-4">
                    ...
                    <div className="text-xs text-muted-foreground">{meeting.participants.length} participants</div>
                  </div>
                ))}
              </div>
            </div>
